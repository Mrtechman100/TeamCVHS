...
from tps.client.pygame_game import PygameGame
from tps.client.client_game_socket import ClientGameSocket
WINDOWS_EINVALID = 10022
...
class PygameSocketGame(PygameGame):
    ...
    def socket_is_ready(self):
        try:
            ...
            if e.errno == WINDOWS_EINVALID:
                self.logger.warning("_read_int: %s", e.strerror)
            else:
                self.logger.error("_read_int: %s", e.strerror)
                raise e
