...
class GameCommException(Exception):
    ...
    def is_bad_command(self):
        return self.msg[:len(E_BAD_CMD)] == E_BAD_CMD
...
class GameComm:
    ...
    def _read_char( self, count ):
        c = self.sock.recv( count )
        if len( c ) > 0:
            c = c.decode("utf-8")
        else:
            self.ok = False
    ...
    def read_mesg(self):
        ...
        try:
            ...
            if code in ALL_MESSAGES:
                ...
                self.logger.debug('read_mesg: msg: %s', msg)
            else:
                self.ok = False
            ...
            if e.is_0_bytes_read():
                ...
                msg = GameMessageBadCommand( e.get_msg() )
            else:
                self.logger.error("Unexpected GameCommException: %s", e)
                raise e
        except socket.error as e:
            if e.errno == errno.EAGAIN or e.errno == WINDOWS_EAGAIN:
                self.logger.debug("read_mesg: %s", e.strerror)
    ...
    def write_mesg(self, msg):
        try:
            ...
            self.logger.debug('write_mesg: msg: %s', msg)
            ...
            if e.errno == errno.EAGAIN or e.errno == WINDOWS_EAGAIN:
                self.logger.warning("write_mesg: %s", e.strerror)
                # we'll assume it's not too important
                # but, it is if it happens repeatedly
                return False
            else:
                raise e
